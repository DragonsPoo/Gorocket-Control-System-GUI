...
[![Version](https://img.shields.io/badge/Version-v2.6.1-orange?style=for-the-badge)](#)
...
### 실제 운용 밸브 매핑
| 밸브 ID | 이름 | 서보 인덱스 | 용도 | 운용 단계 |
|---------|------|-------------|------|-----------|
| SV0 | Ethanol Purge Line | 0 | 에탄올 라인 퍼지 | Phase 1, 5-6 |
| SV1 | Main Pressurization | 1 | 메인 라인 가압 (600 psi) | Phase 3 |
| SV2 | Ethanol Fill Line | 2 | 에탄올 충전 라인 | Phase 2 |
| SV3 | N2O Main Supply | 3 | 산화제 주 공급 | Phase 4 |
| SV4 | Ethanol Main Supply | 4 | 연료 주 공급 | Phase 4 |
| SV5 | System Vent 1 | 5 | 시스템 벤트 1 | Phase 5-6 |
| SV6 | System Vent 2 | 6 | 시스템 벤트 2 | Phase 5-6 |
...
**단일 단계 즉시 실행 (0ms 지연)**:
- ✅ 주 공급 차단: Ethanol Main Supply, N2O Main Supply
- ✅ 가압 차단: Main Pressurization, Ethanol Fill Line
- ✅ 안전 배출: System Vent 1, System Vent 2, Ethanol Purge Line 개방
...
#### 🚀 **운용 절차 시퀀스** *(Fill & Fire Procedure)*
| 단계 | 시퀀스명 | 단계 수 | 소요시간 | 설명 |
|------|----------|---------|----------|------|
| **Phase 1** | Pre-Test Nitrogen Purge | 5 | ~25초 | 시스템 전체 N2 퍼지 (20s) |
| **Phase 2** | *(Manual)* | - | 수동 | 에탄올/N2O 충전 (외부 작업) |
| **Phase 3** | System Pressurization (600 psi) | 4 | 대기형 | 메인 라인 600 psi 가압 |
| **Phase 4** | Hot-Fire Sequence | 7 | ~2.3초 | 연료 선공급 → 점화 → 연소 |
| **Phase 5-6** | Post-Test Safing | 8 | 대기형 | 라인 퍼지 + 압력 해제 |

#### 🛡️ **안전 & 초기화 시퀀스**
| 카테고리 | 시퀀스명 | 단계 수 | 소요시간 | 용도 |
|----------|----------|---------|----------|------|
| **응급** | Emergency Shutdown | 1 | <0.1초 | 즉시 안전상태 |
| **초기화** | Pre-Operation Safe Init | 1 | 즉시 | 모든 밸브 닫힘 확인 |

#### 🔧 **하드웨어 테스트 시퀀스**
| 카테고리 | 시퀀스명 | 단계 수 | 소요시간 | 용도 |
|----------|----------|---------|----------|------|
| **모터 테스트** | Random Test (6 valves) | 12 | ~10초 | SV0-SV5 동작 테스트 |
| **모터 테스트** | Random Test (7 valves) | 14 | ~12초 | SV0-SV6 전체 테스트 |

### 🔥 **실제 Fill & Fire 운용 절차**

#### **Phase 1: Pre-Test Nitrogen Purge** 
```bash
1. PR1을 5 psig 저압으로 설정
2. SV1 (Main Pressurization) 개방
3. SV2, SV0, SV5, SV6 개방하여 시스템 전체 퍼지
4. 20초 이상 N2로 공기/수분/불순물 제거
5. 모든 밸브 닫아 시스템 밀폐
```

#### **Phase 2: 추진제 충전** *(수동 작업)*
```bash
# 에탄올 충전
1. SV2 닫힌 상태에서 BV2, BV4, BV6 개방
2. 딥튜브/니들밸브로 에탄올 250mL 이송
3. 이송 완료 후 모든 밸브 즉시 차단

# N2O 충전  
1. BR1 600 psi 설정 확인
2. N2O 공급장치를 BV5 연결
3. NV1 천천히 열어 0.4kg 충전 (LC1 확인)
4. BV5, NV1 닫고 호스 잔압 제거
```

#### **Phase 3: System Pressurization (600 psi)**
```bash
1. PR1을 600 psi 최종 압력으로 설정
2. SV1 (Main Pressurization) 개방
3. PT1 ≥ 580 psi 도달 대기 (최대 2분)
4. 시스템 압력 안정화 확인
```

#### **Phase 4: Hot-Fire Sequence**
```bash
1. SV3 (N2O) 닫힘 상태 확인
2. SV4 (Ethanol) 먼저 개방 → 연료 선공급
3. 0.15초 지연 (Fuel Lead Time)
4. SV3 (N2O) 개방 → 점화/연소 시작
5. 2초 이상 메인 연소 (모든 센서 데이터 기록)
6. SV3 (N2O) 먼저 차단 → 산화제 차단
7. 0.1초 지연 후 SV4 (Ethanol) 차단
```

#### **Phase 5-6: Post-Test Safing**
```bash
1. SV0 (Ethanol Purge) 즉시 개방 → 라인 퍼지
2. 3초간 N2로 엔진 라인 잔여 에탄올 소화
3. SV5, SV6 (System Vents) 개방 → 압력 해제
4. PT1, PT2 ≤ 15 psi 완전 해제 대기
5. 모든 밸브 닫고 시스템 전원 차단
```
...
```json
{
  "condition": {
    "sensor": "pt1",           // 센서명
    "min": 580,                // 최소값 (600 psi 목표에서 580 psi)
    "op": "gte",              // 연산자 (gte/lte)
...
| 분야 | 변경사항 | 영향 |
|------|----------|------|
| **서보 제어 안정성** | BUSY 체크 복구로 서보 동시 attach() 충돌 해결 | 모든 밸브(V0-V6) 정상 동작 복구 |
| **압력 데이터 전송** | 센서 간 콤마 구분자 누락 수정 | GUI에서 압력값 정상 수신 |
| **CRC 최적화** | 스트리밍 방식 CRC 계산으로 96바이트 버퍼 제거 | 메모리 효율성 및 성능 향상 |
| **GUI 오류 처리** | 시스템 메시지 에러 로그 제거 | 불필요한 오류 메시지 정리 |
...
| 분야 | 변경사항 | 영향 |
|------|----------|------|
| **밸브 매핑** | SV0-SV6 실제 운용 목적으로 재매핑 | Fill & Fire 절차에 완전 부합 |
| **운용 시퀀스** | 4개 새 시퀀스 추가 (Pre-Test → Hot-Fire → Safing) | 실제 로켓 테스트 절차 자동화 |
| **안전 역할** | mains/vents/purges 재정의 | 실제 밸브 기능에 맞는 안전 그룹 |
| **시퀀스 정리** | 레거시 시퀀스 제거, 테스트 시퀀스 유지 | 불필요한 시퀀스 제거로 명확성 향상 |
...
| **로그 무결성** | 세션 메타데이터 + SHA256 해시 | 설정 파일 변경 추적 및 감사 |
| **압력 정책** | 펌웨어-config.json 완전 정렬 | ROC 50 psi/s, Trip 1000 psi 통일 |
...
#### 📋 **실제 운용 밸브 변경사항**
| 이전 (테스트용) | 현재 (실제 운용) | 변경 사유 |
|----------------|-----------------|-----------|
| Ethanol Main | **Ethanol Main Supply** | Phase 4 연료 주 공급 |
| N2O Main | **N2O Main Supply** | Phase 4 산화제 주 공급 |
| Ethanol Purge | **Ethanol Purge Line** | Phase 1,5-6 라인 퍼지 |
| N2O Purge | **Ethanol Fill Line** | Phase 2 충전 라인 |
| Pressurant Fill | **Main Pressurization** | Phase 3 600 psi 가압 |
...
#### 🔧 **v2.6.1 핵심 수정**
- ✅ **서보 모터 문제**: V1(핀12), V3(핀10), V5(핀8) 동작 불능 → BUSY 체크 복구로 해결
- ✅ **압력 데이터 누락**: GUI에서 압력값 못받는 문제 → 콤마 구분자 수정으로 해결  
- ✅ **CRC 성능 최적화**: 96바이트 버퍼 + snprintf 오버헤드 → 스트리밍 방식으로 개선
- ✅ **GUI 에러 로그**: 시스템 메시지가 에러로 출력 → 필터링 로직 개선

#### 🚀 **이전 버전 성과**
- ✅ **실제 운용 전환**: Fill & Fire 절차에 완전 부합하는 밸브 매핑 및 시퀀스
- ✅ **운용 시퀀스 자동화**: Pre-Test → Hot-Fire → Safing 4단계 완전 자동화
- ✅ **시퀀스 정리**: 불필요한 레거시 시퀀스 제거, 필수 테스트 시퀀스만 보존
- ✅ **안전 시스템**: EMERG 시 완전한 전송 중단 및 즉시 복구
- ✅ **FAILSAFE 보호**: 연속 재진입 방지 및 쿨다운 메커니즘
- ✅ **압력 정책 정렬**: 펌웨어-설정 파일 완전 일치 (ROC 50 psi/s, Trip 1000 psi)
- ✅ **로그 무결성**: 세션 메타데이터 + SHA256 해시 + 강제 플러시
